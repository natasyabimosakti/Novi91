// ==UserScript==
// @name         Auto Bet Final v8.91.3
// @namespace    http://tampermonkey.net/
// @version      8.91.3
// @description  Auto Bet dengan protektor, martingale semua kategori, zigzag dozen, column delay, saldo akurat, dan confirm sinkron tanpa bet berulang
// @match        http*://*/*
// @grant        none
...
(function () {
    ...
    setTimeout(() => {
        ...
        function parseRupiah(text) {
            ...
            return parseInt(cleaned.slice(0, -2), 10) || 0;
        }
        ...
        function evaluateBets(winNum) {
            ...
            if (targetDozen !== null) {
                if (d === targetDozen) {
                    console.log(`✅ DOZEN menang di ${d}, reset martingale`);
                    targetDozen = null;
                    fibIndexDozen = 0;
                    zigzagCount = 0;
                } else if (d > 0) {
            ...
            if (zigzagCount >= 6 && !targetDozen && d > 0) {
                targetDozen = d;
            ...
            if (lastTargetColumn.length > 0) {
                if (lastTargetColumn.includes(c)) {
                    console.log(`✅ COLUMN menang di ${c}, reset martingale`);
                    fibIndexColumn = 0;
                    lastTargetColumn = [];
                } else {
        ...
        async function loopMain() {
            ...
            const roundId = resultSpan.textContent.trim() + '_' + history.length;
