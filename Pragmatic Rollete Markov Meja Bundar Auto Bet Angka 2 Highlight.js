...
(function() {
    ...
    // --- CONFIGURATION ---
    const SELECTORS = {
        historyPanel: '.overscrollable', // Panel history besar
        recentResults: '[data-testid="inline-recent-results"]', // Baris hasil terbaru
        itemSelector: '[data-multiplier]', // Angka hasil
        timer: '[data-testid^="timer-countdown-"]', // Selector countdown timer
        raceTrack: '[data-testid="race-track"]', // Container Meja Bundar
    ...
    function initUI() {
        if (ui || document.getElementById('roulette-markov-ui')) return;
    ...
    function highlightRacetrack() {
        ...
        allSpots.forEach(s => {
            ...
            if (rtText) {
                ...
                // Ambil angka murni dari teks DOM
    ...
    function extractNumbers(isNewRoundTrigger) {
        const getNumbersFromContainer = (container) => {
            ...
            return items.map(item => {
                ...
                for (let d of divs) {
                    const val = parseInt(d.textContent.trim());
    ...
    async function executeAutoBet() {
        ...
        // Jeda kecil untuk memastikan UI Racetrack sudah stabil
        ...
        for (const num of state.plannedCenters) {
            // Langkah 1: Cari elemen rtText yang teksnya cocok
            const rtTexts = track.querySelectorAll('.rtText');
            const foundText = Array.from(rtTexts).find(t => t.textContent.trim() === num.toString());
            
            // Langkah 2: Klik pada elemen [data-bet-code] (induk dari rtText)
            const targetClick = foundText ? foundText.closest('[data-bet-code]') : null;

            if (targetClick) {
                console.log(`🎯 Clicking core number: ${num} (verified via text, clicking [data-bet-code])`);
                await bypassClick(targetClick);
                await new Promise(r => setTimeout(r, 150)); // Jeda 150ms antar klik inti
            } else {
                console.warn(`⚠️ Angka inti ${num} tidak ditemukan di racetrack via .rtText.`);
            }
        }
        ...
        if (state.martingaleStep > 0) {
            ...
            const doubleBtn = doubleIcon ? doubleIcon.closest('[role="button"]') || doubleIcon.parentElement : null;
            if (doubleBtn) {
    ...
    setInterval(async () => {
        ...
        // --- LOGIKA INITIAL LOAD ---
        if (freshData.length === 0 && (rawData.panel.length > 0 || rawData.bar.length > 0)) {
